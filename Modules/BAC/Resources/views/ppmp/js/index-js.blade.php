        ...
        columns: [
        	...
        	{data : 'dept_desc', name: 'dept_desc'},
          {
            ...
            render : function(data,type,row){
              var img = '../ppmp/';
              return '<form action="{{route('bac.ppmp_upload_add')}}" method="post" id="form_upload" enctype="multipart/form-data">{{csrf_field()}}\
                      <div class="col-md-9">\
...
$.fn.deletePPMPUpload = function(id){
    ...
    if(x){
      $.ajax({
            type: "POST",
            url: "{{route('bac.ppmp-upload-delete')}}",
            data : {
              upload_id : id,
              _token : '{{csrf_token()}}'
            },
