...
<section class="content">
  <?php for($i = 0; $i < count($navs['main']); $i++): ?>
     <?php
         if(($i%3)==0){
           echo ' <div class="row">';
         }
      ...
      <div class="col-md-4">
       <div class="box ">
            <div class="box-header">
             <h3><?php echo e($navs['group'][$i]->ugrp_name); ?></h3>
              <input type="hidden" id="group_id[<?php echo e($navs['group'][$i]->id); ?>]" value="<?php echo e(count($navs['main'][$i])); ?>" />
             <div class="btn-group">
                      <button type="button" class="btn btn-success" onclick="$(this).addMainMenu('<?php echo e($navs['group'][$i]->id); ?>','<?php echo e($navs['group'][$i]->ugrp_name); ?>')">Add</button>
            </div>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
            ...
            <?php $__currentLoopData = $navs['main'][$i]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $nav): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
              ...
              <?php if($nav->parent==0): ?>
                  <li <?=array_has($prop_main, 'li')?$prop_main['li']:''?>  >
                   <input type="hidden" id="group_id_sub[<?php echo e($navs['group'][$i]->id); ?>][<?php echo e($nav->id); ?>]" value="0" />
                          <a  <?=(array_has($prop_main, 'a')?$prop_main['a']:'')?> href="#"  onclick="$(this).editMainMenu( '<?php echo e($nav->id); ?>' ,  '<?php echo e($navs['group'][$i]->id); ?>' ,  '<?php echo e($navs['group'][$i]->ugrp_name); ?>'   );">
                            <i <?=array_has($prop_main, 'i')?$prop_main['i']:''?> ></i>
                            <?php echo e($nav->title); ?>

                      </a>
                      <button type="button" class="btn btn-info btn-xs" onclick="$(this).childToMainMenu('<?php echo e($nav->id); ?>', '<?php echo e($nav->title); ?>' , '<?php echo e($navs['group'][$i]->id); ?>' , '<?php echo e($navs['group'][$i]->ugrp_name); ?>'  );"><i class="fa fa-plus"></i></button>
                  </li>
              <?php else: ?>
                   <li class="treeview">

                      <a  <?=(array_has($prop_main, 'a')?$prop_main['a']:'')?> href="#" onclick="$(this).editMainMenu('<?php echo e($nav->id); ?>' ,  '<?php echo e($navs['group'][$i]->id); ?>' ,  '<?php echo e($navs['group'][$i]->ugrp_name); ?>');">
                        <i <?=$prop_main['i']?> ></i> <span><?php echo e($nav->title); ?></span>
                      </a>
                       <button type="button" class="btn btn-info btn-xs" onclick="$(this).childToMainMenu('<?php echo e($nav->id); ?>', '<?php echo e($nav->title); ?>'  , '<?php echo e($navs['group'][$i]->id); ?>' , '<?php echo e($navs['group'][$i]->ugrp_name); ?>');"><i class="fa fa-plus"></i></button>
                      <ul class="treeview-menu">
                      <?php $count_sub=0;?>
                      <?php $__currentLoopData = $navs['subs'][$i]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $snav): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <?php if($nav->id==$snav->parent_id): ?>
                         <?php
                          ...
                          <li <?=array_has($prop_sub, 'li')?$prop_sub['li']:''?> >
                                      ...
                                      <a href="#" <?=array_has($prop_sub, 'a')?$prop_sub['a']:''?> onclick="$(this).editChildMainMenu('<?php echo e($snav->id); ?>','<?php echo e($nav->id); ?>' ,  '<?php echo e($navs['group'][$i]->id); ?>' ,  '<?php echo e($navs['group'][$i]->ugrp_name); ?>');">
                                      <i <?=array_has($prop_sub, 'i')?$prop_sub['i']:''?> ></i>
                                      <?php echo e($snav->title); ?>
                      ...
                      <input type="hidden" id="group_id_sub[<?php echo e($navs['group'][$i]->id); ?>][<?php echo e($nav->id); ?>]" value="<?php echo e($count_sub); ?>" />
                  </li>
              <?php endif; ?>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
...
$.fn.editMainMenu = function(id,ugrp_id,ugrp_name){
      ...
      $.ajax({
                  type: "POST",
                  url: "<?php echo e(route('admin.get_main_menus_info')); ?>",
                  data : {
                          main_menu : id,
...
$.fn.submitEditMainNavs = function(){
    $.ajax({
            type: "POST",
            url: "<?php echo e(route('admin.check_main_menus_info')); ?>",
...
$.fn.submitAddMainNavs = function(){
      $.ajax({
                  type: "POST",
                  url: "<?php echo e(route('admin.add_main_menus')); ?>",
...
$.fn.submitAddChildMainNavs = function(){
      $.ajax({
            type: "POST",
            url: "<?php echo e(route('admin.add_child_navigations_main')); ?>",
