             ...
             { data: 'obr_date', name: 'olongapo_obr.obr_date' },
              ...
              render : function(data , type , row){
                      return '<button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#add_purchase_order_modal" onclick="$(this).addPOnumber('+data.pono_id+');" >Update</button>\
                        <form method="post" action="<?php echo e(route('po.po_pdf')); ?>"><?php echo e(csrf_field()); ?><input type="hidden" name="pono_id" value="'+data.pono_id+'" /><input type="submit" class="btn btn-sm btn-default" name="pdf" value="Pdf" /> </form> ';
                }
              },
        ],
        columnDefs: [
...
$('#po_date').on('change',function(){
  ...
  $.fn.sentPurchaseOrder = function(){
      ...
      $.ajax({
            type: "POST",
            url: "<?php echo e(route('po.update_po_records')); ?>",
