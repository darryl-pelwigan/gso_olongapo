                     ...
                     <label for="sof_desc" class="col-sm-2 control-label">BAC RESOLUTION TYPE : </label>
                      <div class="col-sm-4">
                          <select class="form-control" name="bac_reso_type" id="bac_reso_type">
                            <option value=""></option>
                            <?php $__currentLoopData = $procurement; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $procu): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                              <option value="<?php echo e($procu->id); ?>"><?php echo e($procu->proc_title); ?></option>
...
$('#sof_desc').autocomplete({
    serviceUrl: '<?php echo e(route("bac.a_get_sof")); ?>',
    dataType: 'json',
    type: 'POST',
    minChars:0,
    params : {
...
$('#bac_categ').autocomplete({
    serviceUrl: '<?php echo e(route("bac.a_get_categ")); ?>',
    dataType: 'json',
    type: 'POST',
    minChars:0,
    params : {
...
$.fn.sentBACProcesses = function(){
  $.ajax({
    type: "POST",
    url: "<?php echo e(route('bac.process_pr')); ?>",
...
function editDays(date) {
    ...
    $.fn.setControllNumber = function(){
      ...
      var bac_datex = bac_date.split('-');
          $.ajax({
            type: "POST",
             url: "<?php echo e(route('bac.set-bac-control-no')); ?>",
            data : {
              bac_date : bac_date,
  ...
  $.fn.addPOnumber = function(abstrct_id){
    ...
    $.ajax({
            type: "POST",
             url: "<?php echo e(route('bac.get-pr-supplier')); ?>",
            data : {
  ...
  $.fn.deleteBacPONumber = function(control_id){
    ...
    if(x){
        $.ajax({
            type: "POST",
             url: "<?php echo e(route('bac.delete-control-info')); ?>",
            data : {
