            ...
            { data: null, name: 'olongapo_purchase_request_no.pr_date_dept',
                render : function(data, type, row){
                   ...
                   if(data.pr_status === '' || data.pr_status === null){
                       action = '<form method="get" action="{{route('dept.pr_edit')}}">{{csrf_field()}}<input type="hidden" name="pr_id" value="'+data.olongapo_purchase_request_no.id+'" /> <button type="submit" class="btn btn-sm btn-primary" ><i class="fa fa-pencil"></i> Edit</button></form> ';
...
$.fn.sentPurchaseRequest = function(){
    ...
    el.attr('disabled',true);
      $.ajax({
            type: "POST",
            url: "{{route('dept.add_purchase_request')}}",
