        ...
        [
          ...
          { data: null, name: 'olongapo_purchase_request_no.pr_date_dept',
              ...
              if(data.pr_status === '' || data.pr_status === null){
                 action = '<form method="get" action="{{route('pr.pr_edit')}}">{{csrf_field()}}<input type="hidden" name="pr_id" value="'+data.olongapo_purchase_request_no.id+'" /> <input type="submit" class="btn btn-info btn-sm" name="view" value="view" /> <input type="submit" class="btn btn-sm btn-warning" name="edit" value="edit" /> </form> ';
