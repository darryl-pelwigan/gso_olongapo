        ...
        [
          ...
          { data: null, name: 'olongapo_purchase_request_no.pr_date_dept',
              ...
              if(data.pr_status === '' || data.pr_status === null){
                 action = '<form method="post" action="{{route('bac.ppmp_pr')}}">{{csrf_field()}}<input type="hidden" name="pr_id" value="'+data.olongapo_purchase_request_no.id+'" /> <input type="submit" class="btn btn-info btn-sm" name="view" value="view" />  </form> ';
